...
class UserForm(forms.Form):
    ...
    def clean_name(self):
        ...
        if len(name) < 3:
            raise forms.ValidationError('Имя должно содержать не менее 3 символов')# это полезно выводить подсказку
        return name
...
class ManyFieldsForm(forms.Form):
    ...
    is_active = forms.BooleanField(required=False) # False - изначально нет галочки
...
class ImageForm(forms.Form):
    image = forms.ImageField() # если не изображение а файл - FileField()
