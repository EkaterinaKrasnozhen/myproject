...
def post_detail(request, year, month, slug):
    ...
    return JsonResponse(post, json_dumps_params={'ensure_ascii': False}) # не только латиница, а все остальные тоже
...
def author_posts(request, author_id):
    ...
    posts = Post.objects.filter(author=author).order_by('-id')[:5] # -id  в обратном порядке с большего id, :5 самых старых записей сначала
